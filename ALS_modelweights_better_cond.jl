function ALS_modelweights(y::Vector,khr::Vector{Matrix{Float64}},invKuu::MPT{4},rnks::Vector{Int},maxiter,σₙ²::Float64)
    ...
    ###########################################################################
        ...
        for iter = 1:maxiter
            for k = 1:2D-2
                ...
                # update dth tt-core
                Wd          = getU(tt,d)
                prior       = mpo2mat((transpose(Wd)*invKuu)*Wd);
                tmp         = [ΦWd/sqrt(σₙ²); cholesky(Hermitian(prior)).L]
                tt[d]       = reshape(tmp\[y/sqrt(σₙ²); zeros(size(prior,1))],size(tt0[d])) 
                # compute residual
